...
function showall()
{
  ...
  for(i=0; i<len; i++)
  {
    let display = document.createElement("view");
    display.innerHTML = generateRowHTML(v[i]);
...
{
  document.getElementById("results").innerHTML = "";
  const input = document.getElementById('txt-search').value;
  if(input==""){alert("Please enter a keyword");}
  else{
  let count=0;
  for(i=0;i<len;i++)
  {
    var str=v[i].description;
  ...
  {
      let display = document.createElement("view");
      display.innerHTML = `No videos found with keyword ${input}`;
      document.getElementById("results").appendChild(display);
  }
